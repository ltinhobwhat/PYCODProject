...
def save_progress(user_id, score, completed):
    ...
    cursor.execute('''
        SELECT is_completed, best_score, total_attempts 
        FROM game_progress 
    ...
    if current:
        ...
        already_completed = current[0]
        best_score = max(current[1], score)
        total_attempts = current[2] + 1
        
        cursor.execute('''
            UPDATE game_progress 
            SET is_completed = ?, best_score = ?, total_attempts = ?
            WHERE user_id = ? AND game_name = ?
        ''', (completed or already_completed, best_score, total_attempts, user_id, 'quiz'))
        
        # Only update total score if this is a better score AND not already completed
        if score > current[1] and not already_completed and completed:
            score_difference = score - current[1]
            cursor.execute('''
                UPDATE users 
                SET total_score = total_score + ?
                WHERE id = ?
            ''', (score_difference, user_id))
    else:
        # First time playing
        cursor.execute('''
        ...
        # Add score to total if completed
        if completed:
            cursor.execute('''
                UPDATE users 
...
def result():
    ...
    completed = score > 0  # CHANGÉ: Marquer comme complété si au moins 1 bonne réponse
    ...
    # Save progress - TOUJOURS sauvegarder, même avec 0
